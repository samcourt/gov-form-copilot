...
export function extractEvidenceFromText(args: {
  ...
  if (args.sourceType === "birth_certificate") {
    assignIfFound(values, "student.givenName", args.rawText, [/given names?\s*[:\-]\s*([A-Za-z][A-Za-z\s'-]+)/i]);
    assignIfFound(values, "student.familyName", args.rawText, [/family name\s*[:\-]\s*([A-Za-z][A-Za-z\s'-]+)/i, /surname\s*[:\-]\s*([A-Za-z][A-Za-z\s'-]+)/i]);
    assignIfFound(values, "student.dateOfBirth", args.rawText, [/date of birth\s*[:\-]\s*(\d{1,2}[\/\-]\d{1,2}[\/\-]\d{4})/i, /dob\s*[:\-]\s*(\d{1,2}[\/\-]\d{1,2}[\/\-]\d{4})/i], normaliseAustralianDate);
    assignIfFound(values, "student.countryOfBirth", args.rawText, [/country of birth\s*[:\-]\s*([A-Za-z][A-Za-z\s'-]+)/i, /place of birth\s*[:\-]\s*([A-Za-z][A-Za-z\s'-]+)/i]);
  }

  if (args.sourceType === "utility_bill") {
    assignIfFound(values, "address.fullAddress", args.rawText, [/service address\s*[:\-]\s*(.+)/i, /supply address\s*[:\-]\s*(.+)/i, /address\s*[:\-]\s*(.+)/i]);
